...
let
    ...
    Продажи = Table.SelectRows(ДП_Продажи, each [Период] > ПоследняяДатаИстории),
    Склад = Table.SelectRows(fnДП_Остатки("остатки"), each [Период] > ПоследняяДатаИстории),
    Путь = Table.SelectRows(fnДП_Остатки("товар в пути"), each [Период] > ПоследняяДатаИстории),
